...
{
    public NavCommunicationEmulator(string name) : base(name, "via Navigator", 0, CommandStringFormat.Ascii)
    {
...
public abstract class NavDeviceBase : AVoIPEndpoint
{
        ...
        base(name, deviceType, new NavCommunicationEmulator(GetCommunicationClientName(deviceType, name)))
    {
        ...
        CommunicationClient.ConnectionStateHandlers += x => DeviceConnectionState = x;
        PollWorker = new ThreadWorker(PrePoll, TimeSpan.FromSeconds(90));
        PollWorker.Restart();
    }
